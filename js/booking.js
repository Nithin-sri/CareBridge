...
function selectSession(sessionName, btn) {
  ...
  // Clear any previous error on session type
  document.getElementById('sessionType')?.classList.remove('error');
  document.getElementById('sessionTypeError')?.classList.remove('visible');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  fields.forEach(({ field, error }) => {
    ...
    el?.addEventListener('input',  () => clearError(field, error));
    el?.addEventListener('change', () => clearError(field, error));
  });
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      ...
      if (!consent1?.checked) {
        document.getElementById('consent1Error')?.classList.add('visible');
        valid = false;
      } else {
      ...
      if (!consent2?.checked) {
        document.getElementById('consent2Error')?.classList.add('visible');
        valid = false;
      } else {
      ...
      if (!valid) {
        document.querySelector('.error, .form-error.visible')
